...
export default function ChatPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("harmoni_conversations");
    if (saved) {
      const parsed = JSON.parse(saved);
      const formatted = parsed.map((conv) => ({
        ...conv,
      ...
      messages: [
        {
          id: "init",
          role: "model",
          content: "Hello! I'm Harmoni, your AI therapeutic companion. I'm here to listen and support you. How are you feeling today?",
          timestamp: new Date(),
        },
      ],
    };
  ...
  const handleSendMessage = async () => {
    ...
    const updatedConversations = conversations.map((conv) => {
      if (conv.id === activeConversationId) {
        ...
        const isFirstUserMessage = conv.messages.length === 1 && conv.messages[0].role === "model";
        const newTitle = isFirstUserMessage ? generateConversationTitle(currentInputValue) : conv.title;
